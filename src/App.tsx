...
export default function App() {
  ...
  const handleEmail = () => {
    const subject = encodeURIComponent(`Facture Conforama ${invoiceNumber}`);
    const body = encodeURIComponent(`Bonjour,\n\nVeuillez trouver ci-joint les détails de votre facture ${invoiceNumber} pour un montant total de ${total.toFixed(2)} CHF.\n\nCordialement,\nService Client Conforama Suisse`);
      ...
      <div className={`w-full max-w-[210mm] shadow-2xl transition-all duration-300 ${isPrinting ? 'shadow-none' : ''}`}>
        ...
        {!isPrinting && (
          <div className="fixed bottom-8 right-8 z-50 flex flex-col md:flex-row gap-4">
            <button 
              onClick={handleEmail}
              className="flex items-center gap-3 px-6 py-4 bg-slate-800 text-white rounded-full font-bold shadow-2xl hover:scale-105 active:scale-95 transition-all"
              title="Envoyer par e-mail"
            >
              <Mail className="w-6 h-6" />
              <span className="hidden md:inline uppercase tracking-widest text-xs">Envoyer par Email</span>
            </button>
            <button 
              onClick={handlePrint}
